...
FUNCTION read_do25model_from_path,readlines,path,prefix=prefix
...
;; elements in it
FUNCTION read_do25model_match_strings,sLong,sFind
...
;; to that parameter
FUNCTION read_do25model_values_from_param,allparams,allvalues,param,numeric=numeric
...
FUNCTION read_do25model_read_batch,filename
...
PRO read_do25model_parse_file,alllines,separator=separator, $
...
;; now extract the paths
  ...
  FOR i =0,n_elements(uniq_params)-1 DO BEGIN
      idx[i] = (n_elements(read_do25model_values_from_param(params,values,uniq_params[i])) GT 1)
  ENDFOR
...
PRO read_do25model_Event, sEvent
      ...
      'COMBOBOX': BEGIN
          ...
          IF sEvent.Index NE 0 THEN BEGIN
              sInfo.selectioncrit[comboidx] = sInfo.uniq_params[comboidx]+sInfo.separator+ $
                (read_do25model_values_from_param(sInfo.params,sInfo.values,sInfo.uniq_params[comboidx]))[sEvent.Index-1]
          ENDIF ELSE BEGIN
          ...
          IF newcrit NE oldcrit THEN BEGIN
              ...
              keep = read_do25model_match_strings(sInfo.paths,sInfo.selectioncrit)
          ...
          IF keep[0] NE -1 THEN BEGIN
              ...
              ENDIF ELSE BEGIN
                  ...
                  IF command NE '' THEN BEGIN
                      model = read_do25model_from_path(sInfo.Readlines, $
...
FUNCTION read_do25model,filename,command=command,separator=separator, $
...
;; routing instead of using the standard plotting device if any
  ...
  ;; now read the file
  batchcontent = read_do25model_read_batch(filename)
...
;; path of the current working dir
  ...
  read_do25model_parse_file,batchcontent,separator=separator, $
    command=command, readlines=readlines, models=models, paths=paths, $
    params=params, values=values, uniq_params=uniq_params
...
;; the menu structure
  ...
  FOR i=0,n_elements(uniq_params)-1 DO BEGIN
      ...
      wcombobox = widget_droplist(wparamcolumn,uvalue='COMBOBOX'+n2s(i), $
                                  value=['Any', read_do25model_values_from_param(params,values,uniq_params[i])])
  ENDFOR
  ...
  wcommandtext = widget_text(wbase, /editable, value='pldo25,%m.full', $
                             uvalue='COMMAND')
...
;; store the sinfo in the uvalue of the top widget
  ...
  xmanager, "read_do25model_", wbase, $
            event_handler="read_do25model_event", $
            no_block=no_block
  ...
  IF no_block EQ 0 THEN BEGIN
      ...
      ENDIF ELSE BEGIN
          ...
          ENDIF ELSE BEGIN
              path = formdata.path
              Ptr_Free, ptrToFormData
              out = read_do25model_from_path(readlines,path,prefix=PathPrefix)
          ENDELSE
      ENDELSE
      return,out
  ENDIF
  
END                             ; of Read_Do25model
