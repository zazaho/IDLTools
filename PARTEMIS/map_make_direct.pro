...
error_otf_scan=error_otf_scan, parang0=parang0, project=project, mapradecstr, removedrift=removedrift
...
ind = where((abs(offset_str.scan_min_longoff-offset_str.med_scan_min) lt 60.) and (abs(offset_str.scan_max_longoff-offset_str.med_scan_max) lt 60.), count)
print, "Taking ", count, " subscans into account out of a total of ", n_elements(subscan_list)
...
for i = 0, n_elements(subscan_list)-1 do subscan_list(i) = (strsplit(subscan_list(i),'.',/extract))(0)
...
for i = 0, n_elements(subscan_list)-1 do begin
	...
	filename=otf_dat+'otf_subscan_nodrifts_'+strtrim(string(scan_number),1)+'_'+subscan_list(i)+'.xdr'
	endelse
...
;	 
	...
	ENDELSE
        ...
        IF (abs(offset_str.scan_min_longoff(i,iscan)-offset_str.med_scan_min(iscan)) lt 60.) and (abs(offset_str.scan_max_longoff(i,iscan)-offset_str.med_scan_max(iscan))$
	 lt 60. and error_otf_scan(i) eq 0) THEN BEGIN
	
	IF donnees.movefram ne 1 and donnees.scantype ne 'POINT' THEN BEGIN
...
if count ne 0 then map_radec(id) = map_radec(id)/weight_radec(id)
print, numb_scan
if numb_scan gt 1 then begin
obsname = 'APEX' 
for i = 0, n_elements(scan_list)-1 do obsname = obsname+'-'+strtrim(string(scan_list(i)),2)

mapradecstr = {name:obsname,image:map_radec, weight:weight_radec,crpix1:crpix1, crpix2:crpix2, $
