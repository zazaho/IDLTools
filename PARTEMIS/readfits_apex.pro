...
if nbre_1 eq 0 or nbre_2 eq 0 then begin
       ...
       printf, lun, strcompress(string(subscan)), '  : LES FICHIERS ARRAYDATA, DATAPAR ET SCAN N ONT PAS ETE TROUVES  :' + path + obsname
...
u=mrdfits(calibration_camera(0),1)
taille=size(u)
...
toto=mrdfits(calibration_camera(0),1, header_cam)
...
fxaddpar, header_tel, 'VHBLIND',  fxpar(header_cam, 'VHBLIND')
...
'CRVAL1  ', $; 	 258.380402937614d0 / [deg] Native frame zero in basis system (long.)
'CRVAL2  ', $; 	 -22.283130251835d0 / [deg] Native frame zero in basis system (lat.)
...
'BLONGOBJ', $; 	  258.38040090495d0 / [deg] Source longitude in basis frame
'BLATOBJ ', $; 	-22.2831301671406d0 / [deg] Source latitude in basis frame
...
'VL',       $;         		V low  detecteur
'VH',        $;			V high detecteur
'VHBLIND'   $;			V compensation mode differentiel
]
...
sub_com = strmid(commande,strpos(commande,'BLONGOBJ:'),50)
sub_com_ext = strmid(sub_com,0,strpos(sub_com,','))+'d0'+strmid(sub_com,strpos(sub_com,','),strlen(sub_com)-strpos(sub_com,','))
commande_ext1 = strmid(commande,0,strpos(commande,'BLONGOBJ:')+strpos(sub_com,','))+'d0'
commande_ext2 = strmid(commande,strpos(commande,'BLONGOBJ:')+strpos(sub_com,','),strlen(commande)-strpos(commande,'BLONGOBJ:')-strpos(sub_com,','))
commande = commande_ext1+commande_ext2
;
sub_com = strmid(commande,strpos(commande,'BLATOBJ :'),50)
sub_com_ext = strmid(sub_com,0,strpos(sub_com,','))+'d0'+strmid(sub_com,strpos(sub_com,','),strlen(sub_com)-strpos(sub_com,','))
commande_ext1 = strmid(commande,0,strpos(commande,'BLATOBJ :')+strpos(sub_com,','))+'d0'
commande_ext2 = strmid(commande,strpos(commande,'BLATOBJ :')+strpos(sub_com,','),strlen(commande)-strpos(commande,'BLATOBJ Ò:')-strpos(sub_com,','))
commande = commande_ext1+commande_ext2
...
datastr.elevatio = median(datastr.datapar.elevatio)
...
;if check_data eq -1 then return, -1
...
;	power_map2007, datastr.offset, datastr.vh-datastr.vl, $			; 2007
	power_map, datastr.vhblind, datastr.vh-datastr.vl, $			; 2009
