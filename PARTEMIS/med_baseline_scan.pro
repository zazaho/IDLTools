...
; PURPOSE:
;
;	Subtract the median value to the signal and give rms noise.
;
; CALLING SEQUENCE:
;	
;	MED_BASELINE_SCAN, Cube, Cube_corrige, Rms_noise
;
; INPUTS:
;
;	Cube:	Initial data.		
...
pro med_baseline_scan, cube, cube_corrige, rms_noise, cube_masque = cube_masque

; subtract the median value to the signal and calculate rms noise 


if n_params() eq 0 then begin
	print, 'Calling sequence : '
	print, '	MED_BASELINE_SCAN, cube, cube_corrige, rms_noise'
...
for i=0, taille(1)-1 do begin
	...
	if keyword_set(cube_masque) then begin
	   ...
	   if count le 10 then begin	   
             print, 'MED_BASELINE_SCAN : WARNING : Not enough baseline avoiding mask_base for pixel : ', i, j, " Subtracting global median baseline"
	     count = taille(3)
	     id = lindgen(count)
	   endif
	endif
