pro	trilorentzfitplus_funct,x,p,f,pder
; NAME:
;	trilorentzfit_FUNCT
; 
; CATEGORY:
...
; PROCEDURE:
;	f(x)=p(0)*p(2)^2/4/[(x-p(1))^2+p(2)^2/4]+p(3)*p(5)^2/4/[(x-p(4))^2+p(5)^2/4]+p(6)*p(8)^2/4/[(x-p(7))^2+p(8)^2/4]+p(9)+p(10)*x
;
...
;
 ...
 Function trilorentzfit_plus, x, y, errory=errory, p, sigmad
;+
; NAME:
...
; PURPOSE:
; 	Fit y=f(x) where:
;	f(x)=p(0)*p(2)^2/4/[(x-p(1))^2+p(2)^2/4]+p(3)*p(5)^2/4/[(x-p(4))^2+p(5)^2/4]+p9+p10*x
; 	
...
lfit = mycurvefit(x,y,weight,p,sigmad, $
		function_name ='trilorentzfitplus_funct') ;call mycurvefit
;
print, ''
...
print, 'rechte= a+bx met a=',p(9),  'sigma', sigmad(9)
print, '                 b=',p(10),  'sigma', sigmad(10)
return, lfit
end
