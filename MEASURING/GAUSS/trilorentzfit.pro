pro	trilorentzfit_funct,x,p,f,pder
; NAME:
;	trilorentzfit_FUNCT
; 
; CATEGORY:
...
; PROCEDURE:
;	f(x)=p(0)*p(2)^2/4/[(x-p(1))^2+p(2)^2/4]+p(3)*p(5)^2/4/[(x-p(4))^2+p(5)^2/4]+p(6)*p(8)^2/4/[(x-p(7))^2+p(8)^2/4]
;
...
;
 ...
 Function trilorentzfit, x, y, errory=errory, p, sigmad
;+
; NAME:
...
; PURPOSE:
; 	Fit y=f(x) where:
;	f(x)=p(0)*p(2)^2/4/[(x-p(1))^2+p(2)^2/4]+p(3)*p(5)^2/4/[(x-p(4))^2+p(5)^2/4]
; 	
...
lfit = mycurvefit(x,y,weight,p,sigmad, $
		function_name ='trilorentzfit_funct') ;call mycurvefit
;
print, ''
