...
function lineint4, aar,XRANGE = xrange, OLDDATA=olddata, CONT=cont
  ...
  if keyword_set(xrange) then begin
    aar = sh_select(aar, aar.data.wave ge xrange(0) and aar.data.wave le xrange(1)) 
  endif
...
;FWHM = A(2)*2*sqrt{2*ln(2)}  = A(2)*2.35482   
  ...
  print,'Resolving Power: ',A(1)/(A(2)*2.35482) 
