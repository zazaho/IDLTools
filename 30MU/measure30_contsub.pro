function measure30_contsub,in,cont,zoom=zoom,_extra=_extra
  
  smooth_in = cutlines(in)
  
  IF n_elements(cont) NE 0 THEN BEGIN
      f = sh_select_range(subtract(sh_cut(in),cont,/qui),xr=[15,50],/quiet)
      smooth_f = sh_select_range(subtract(sh_cut(smooth_in),cont),xr=[15,50],/quiet)
  ENDIF ELSE BEGIN 
      f = in
      smooth_f = in
  ENDELSE 
  ...
  pl,f,/flambda,/auto,ymin=0,_extra=_extra
  pl,smooth_f,/oplot,thi=3,nsum=5,ps=0,_extra=_extra
  oplot,central_wave*[1,1],!y.crange
...
;;  print,'Integrated Flux',totalflux*1e-14,' W/m/m'
  print,'---------------------------------------------------------'
  print,object(in)
  print,'peak          :',ypeak,' at ',xpeak,' mu'
  print,'FWHM               :',xhright-xhleft, ' mu, from ',xhleft,' to ',xhright,' mu'
  print,'Feature center     :',central_wave, ' mu'
  
  IF n_elements(cont) NE 0 THEN BEGIN
      flux = sh_integrate(f,/noplot,/quiet)
      print,'Integrated Flux:',flux,' W/m/m'
  contatpeak=(shc_interpol(cont.data.wave,cont.data.flux,xpeak))[0]* $
    3d14/1d26*1d13/xpeak^2d0
  ENDIF ELSE BEGIN
      flux = 0d0
      contatpeak=0d0
  ENDELSE
  ...
  return,{feature_30_props,name:object(in),xpeak:xpeak,ypeak:ypeak, $
          fwhm:xhright-xhleft,flux:flux,cpeak:central_wave, $
          contatpeak:contatpeak}
end
