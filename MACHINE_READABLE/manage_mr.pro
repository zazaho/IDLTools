...
PRO manage_mr_disp_text_evt, event
...
PRO manage_mr_disp_text, Text, Title=inTitle

  HEIGHT = 24 
  WIDTH = 80  
  IF keyword_set(inTitle) THEN BEGIN
      TITLE = 'Manage_mr:'+inTitle
  ENDIF ELSE BEGIN
      Title = 'Manage_mr'
  ENDELSE 
...
; Default Done button name:
  ...
  Xmanager, "manage_mr_disp_text", $ ;register it with the
            filebase, $         ;widget manager
            GROUP_LEADER = GROUP, $
            EVENT_HANDLER = "manage_mr_disp_text_evt", /NO_BLOCK
  
END

;; Make the changes to the original structure permanent
PRO manage_mr_apply_changes
  
  COMMON COMMON_manage_mr,sInfo,struct,sColInfo
...
PRO manage_mr_index_zero, wHandle
  manage_mr_set_list_index, 0
END      

;--------------------------------------------------------------------
;
PRO manage_mr_show_sample, wHandle
  COMMON COMMON_manage_mr,sInfo,struct,sColInfo
...
;--------------------------------------------------------------------
;
PRO manage_mr_set_list_index, Index
  ...
  manage_mr_show_sample, sInfo.wSample
END      

;--------------------------------------------------------------------
;
PRO manage_mr_store_edit_data
  COMMON COMMON_manage_mr,sInfo,struct,sColInfo
...
PRO manage_mr_Event, sEvent
  ...
  CASE eventUValue OF
      'LIST': BEGIN
          ...
          manage_mr_store_edit_data 
          
          ;; Update the display with the new selected column
          manage_mr_set_list_index, sEvent.Index
      END
...
;; Update the display with the new selected column
      ...
      'DELETE' : BEGIN
          ...
          IF nactive GT 1 THEN BEGIN
              ...
              manage_mr_set_list_index, sInfo.CurrentIndex < (nactive-2)
          ENDIF
      END 
      ...
      'SCRIPT' : BEGIN
          ...
          IF n_elements(scripttxt) GT 1 THEN begin
              manage_mr_disp_text,scripttxt[1:*],title='script'
          ENDIF
      END
      ...
      'HELP' : BEGIN
          ...
          htxt = [htxt,"restructured = manage_mr(struct)"]
          htxt = [htxt,""]
          ...
          manage_mr_disp_text,htxt,title='Help'
      END
...
FUNCTION manage_mr,instruct,help=help
  
  COMMON COMMON_manage_mr,sInfo,struct,sColInfo

  ;; This is a routine to manage mr structures in a graphical
  ;; environment. It's main aim is to have an interactive tool
  ;; to rename or define columns units and descriptions.
  ;;
  ;; We need to do two things.
  ;; 1) check if the input is already a mr structure. If not if it is
  ;; an array or a structure assume that those are the data columns.
  ;; Try and cast it into a mr-struct. If it is a structure assume
  ;; that the names of the tags hold the column names 
  ;; 2) Create an interfact that allows to select a column and change
  ;; the names, units and descriptions. 
...
;; The menu structure
  ...
  wList = WIDGET_LIST(wLeftBase, /FRAME, NOTIFY_REALIZE='manage_mr_index_zero', $
                          VALUE='Col'+ $
  ...
  wSample = WIDGET_LIST(wRightBase, /FRAME, NOTIFY_REALIZE='manage_mr_show_sample', $
  ...
  ;;
  XMANAGER, "manage_mr_", wTopBase, $
            EVENT_HANDLER="manage_mr_Event"
