...
PRO cmg_write_acf_info_file,dir
...
;; routine to make a cluster catalog simulation and run an
...
PRO cluster_make_grid, $
  ...
  IF keyword_set(help) THEN BEGIN
     print,' PRO cluster_make_grid, [options]'
  ...
  ;; read global variables
  @grid_global_definitions.idl
  calculate_batch_filename = 'cluster_grid_acf_batch.idl'
  ...
  cd,'./',current=pwd
  default,rootdir,pwd+'/CLUSTER_GRID'
...
;; Make the dirnames once here
  ...
  dir2  = make_array(n_elements(CORERADIUS),value='')
  dir3  = make_array(n_elements(GAMMA),value='')
  
  FOR j1 = 0, n_elements(dir1)-1 DO dir1 [j1 ] = 'NSTARS'+par_val_seperator+f2s(NSTARS[j1],0)
  FOR j2 = 0, n_elements(dir2)-1 DO dir2 [j2 ] = 'CORERADIUS'+par_val_seperator+f2s(CORERADIUS[j2],2)
  FOR j3 = 0, n_elements(dir3)-1 DO dir3 [j3 ] = 'GAMMA'+par_val_seperator+f2s(GAMMA[j3],2)
  ...
  FOR j1 = 0, n_elements(dir1)-1 DO BEGIN
     FOR j2 = 0, n_elements(dir2)-1 DO BEGIN
        FOR j3 = 0, n_elements(dir3)-1 DO BEGIN
           ...
           ENDIF ELSE BEGIN
              cmg_create_catalog,dir+'/'+catalogfile_prefix+'.data', $
                                 NSTARS=NSTARS[j1],$
                                 CORERADIUS=CORERADIUS[j2],$
                                 GAMMA=GAMMA[j3]
              
              
              cmg_write_acf_info_file,dir
