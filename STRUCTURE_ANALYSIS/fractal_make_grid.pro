...
PRO hmg_write_acf_info_file,dir
...
PRO fractal_make_grid, $
  ...
  IF keyword_set(help) THEN BEGIN
     print,' PRO fractal_make_grid, [options]'
  ...
  ;; read global variables
  @grid_global_definitions.idl
  calculate_batch_filename = 'fractal_grid_acf_batch.idl'
  ...
  cd,'./',current=pwd
  default,rootdir,pwd+'/FRACTAL_GRID'
...
;; Make the dirnames once here
  ...
  FOR j1 = 0, n_elements(dir1)-1 DO BEGIN
     FOR j2 = 0, n_elements(dir2)-1 DO BEGIN
        ...
        ENDIF ELSE BEGIN
           hmg_create_catalog,dir+'/'+catalogfile_prefix+'.data', $
                              NSTARS=NSTARS[j1],$
                              FRACTALDIMENSION=FRACTALDIMENSION[j2], $
                              _extra=_extra
           
           hmg_write_acf_info_file,dir
