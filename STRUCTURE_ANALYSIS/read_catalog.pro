;; read a catalog table from the give directory.
;; by default it will read the catalog.data file but you can overrule
;; that by doing:
;; catalog = read_catalog("./",prefix="catalog_spl1")
;;
;; (SH Oct 16 2013) deactivated file_decompress
;; (SH Oct 16 2013) Added option to specify prefix

function read_catalog,dir,prefix=prefix

  ;; read global variables
  @grid_global_definitions.idl
  default,prefix,catalogfile_prefix

;  oldquiet = !quiet
;  !quiet = 1
;  tmp = file_decompress(dir+'/'+catalogfile_prefix+'.data*',status=status)
;  !quiet=oldquiet
...
;  file_decompress,/delete
  ...
  if file_test(full_path) then begin
     data=(read_ascii(full_path,comment='#')).(0)
  endif else begin
     data = !values.d_nan
  endelse
